...
public class ListeDesanonymat implements Observer {
	...
	private synchronized void createRapport(MartList<EleveClasse> listeSalle,
		...
		htmlBody += "<table width=100% class='tabSB'>" + "<tr id='ficheNote'>"
				...
				+ "<td class='tdSBLC' width=20%>Prénoms</td>"
				+ "<td class='tdSBLC' width=30%>Etablissement</td>"
				+ "<td class='tdSBLC' width=20>Num. Anonymat</td></tr>";
		...
		for (EleveClasse elv : listeSalle) {
			try {
				htmlBody += "<tr id='ficheNote'>" + "<td class='tdSBT'>"
						...
						+ elv.getEts() + "</td><td class='tdBInf'></td></tr>";
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		preview.addObserver(ListeDesanonymat.this);
	...
	public static void main(String[] args) {
		...
		ListeDesanonymat fr = new ListeDesanonymat();
