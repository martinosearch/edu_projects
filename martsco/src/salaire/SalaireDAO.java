...
public class SalaireDAO extends DAO<AgentSalaire> {
	...
	public boolean createObj(AgentSalaire obj) {
		...
		if (!salAgent.getCodeInfo().equals("")) {
			try {
				...
				String query = "INSERT INTO " + tableSalaire
	...
	public boolean update_create(AgentSalaire obj) {
		...
		if (salAgent.getSerieVersement() * 2 + 4 >= nbreColumn) {
			...
			String query2 = "ALTER TABLE " + tableSalaire + " ADD vers"
	...
	public boolean updateObj(AgentSalaire obj) {
		...
		try {
			Statement state = MartConnection.getInstance().createStatement();
			String query = "UPDATE " + tableSalaire + " SET " + strQuery
					+ " WHERE codeinfo_agent='" + salAgent.getPrimaryKey()
					+ "'";
			System.out.println(query);
			state.executeUpdate(query);
	...
	public boolean deleteObj(AgentSalaire obj) {
		String query = "DELETE FROM table_salaire WHERE " + "codeinfo_agent='"
				+ obj.getPrimaryKey() + "'";
		updateDB(query);
		return false;
	}
	...
	public MartList<AgentSalaire> getList() {
		...
		try {
			Statement state = MartConnection.getInstance().createStatement(
					ResultSet.TYPE_SCROLL_INSENSITIVE,
					ResultSet.CONCUR_READ_ONLY);
			String query = "SELECT * FROM " + tableSalaire
					+ " ORDER BY nom_agent";
