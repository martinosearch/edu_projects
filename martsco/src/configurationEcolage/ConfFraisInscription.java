...
public class ConfFraisInscription extends AbstractConfigPanel {
	...
	public ConfFraisInscription(String tit) {
		super(tit);
	...
	public void refresh() {
		...
		for (Niveau niv : niveaux) {
			String value = "";
			try {
				value = (String) ((Setting) setdao.findObj("ins_"
			...
			sets.add(new Setting("ins_" + niv.getIntitule(), value));
	...
	public void save() {
		for (int i = 0; i < tableInfo.getRowCount(); i++) {
			try {
				String set = "ins_" + tableInfo.getValueAt(i, 0);
